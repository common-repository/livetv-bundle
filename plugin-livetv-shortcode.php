...
function livetv_do_all_livestream_shortcode( $atts, $content = null )
{
	...
	{
		do_livestream_only_from_type($type, $channel, $width, $height, $color, $autoplay);
	}
	else
	{
...
function do_livestream_only_from_type($type, $channel, $width, $height, $color, $autoplay){
		...
		switch($type):
			...
			case 'own3d': //ok
				echo '<object type="application/x-shockwave-flash" width="'.$width.'" height="'.$height.'"><param name="movie" value="http://www.own3d.tv/livestream/'.$channel.';autoplay='.$autoplay.'" /><param name="allowscriptaccess" value="always" /><param name="allowfullscreen" value="true" /><param name="wmode" value="'.$color.'" /><embed src="http://www.own3d.tv/livestream/'.$channel.';autoplay='.$autoplay.'" type="application/x-shockwave-flash" allowfullscreen="'.$autoplay.'" allowscriptaccess="always" width="'.$width.'" height="'.$height.'" wmode="'.$color.'"></embed></object>';
			break;
			
			case 'twitch': //ok
				echo '<object type="application/x-shockwave-flash" height="'.$height.'" width="'.$width.'" id="live_embed_player_flash" data="http://www.twitch.tv/widgets/live_embed_player.swf?channel='.$channel.'" bgcolor="'.$color.'"><param name="allowFullScreen" value="true" /><param name="allowScriptAccess" value="always" /><param name="allowNetworking" value="all" /><param name="movie" value="http://www.twitch.tv/widgets/live_embed_player.swf" /><param name="flashvars" value="hostname=www.twitch.tv&channel='.$channel.'&auto_play='.$autoplay.'&start_volume=25" /></object>';
			break;
			
			case 'justin': //ok
				echo '<object type="application/x-shockwave-flash" height="'.$height.'" width="'.$width.'" id="live_embed_player_flash" data="http://www.justin.tv/widgets/live_embed_player.swf?channel='.$channel.' bgcolor="'.$color.'"><param name="allowFullScreen" value="true" /><param name="allowscriptaccess" value="always" /><param name="allownetworking" value="all" /><param name="movie" value="http://www.justin.tv/widgets/live_embed_player.swf" /><param name="flashvars" value="channel='.$channel.'&auto_play='.$autoplay.'&start_volume=25" /></object>';
			break;
			
			case 'ustream': //ok
				echo '<object width="'.$width.'" height="'.$height.'" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"><param name="flashvars" value="cid='.$channel.'&autoplay='.$autoplay.'"/><param name="allowfullscreen" value="true"/><param name="allowscriptaccess" value="always"/><param name="src" value="http://www.ustream.tv/flash/viewer.swf"/><param name="wmode" value="'.$color.'" /><embed flashvars="cid='.$channel.'&autoplay='.$autoplay.'" width="'.$width.'" height="'.$height.'" allowfullscreen="true" allowscriptaccess="always" src="http://www.ustream.tv/flash/viewer.swf" type="application/x-shockwave-flash"></embed></object>';
			break;
			
			case 'livestream': //ok
				echo '<iframe src="http://cdn.livestream.com/embed/'.$channel.'?layout=4&autoPlay='.$autoplay.'&mute=false&allowchat=false&width='.$width.'&height='.$height.'" width="'.$width.'" height="'.$height.'" style="border:0;outline:0" frameborder="0" scrolling="no"></iframe>';
			break;
			
			default:
				echo '<div id="nok" style="width:'.$width.'px;height:'.$height.'px;border:3px solid '.$color.'">Error</div>';
			break;
